    ...
    <div style="background-color:#073C21;height:671px">
        ...
        <!-- <button class="custom-button" id="Login-button" style="margin-top:40px;margin-left:5%;background-color:#FFFFFF; color:#5F5D5E">Login</button> -->
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                    @auth
                        <button href="{{ url('/home') }}" style="border-radius:15px;height: 40px;width: 100px;
    font-weight: bold;" class="text-sm text-gray-700 dark:text-gray-500">{{ Auth::user()->name }}</button>
                    @else
                        <a href="{{ route('login') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Log in</a>

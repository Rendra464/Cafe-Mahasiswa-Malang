...
<body>
    ...
    <section class="py-16 px-6 bg-[#f9f9f9] min-h-screen">
        <div class="max-w-6xl mx-auto">
            <h2 class="text-3xl font-bold text-center text-[#073C21] mb-10">Menu</h2>
    ...
    <div id="mini-cart" class="hidden fixed bottom-20 right-4 bg-white w-80 max-h-[80vh] overflow-y-auto rounded-xl shadow-lg p-4 border border-gray-200 z-50">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    products.dataproduct.forEach(product => {
        ...
        card.innerHTML = `
            ...
            <p class="text-gray-600 text-sm font-semibold mb-4">Rp ${product.price}</p>
            <button class="bg-[#0B7941] text-white px-4 py-2 rounded-full hover:bg-[#096636] transition duration-300 order-btn">
                Add to Cart
            </button>
        `;

        card.querySelector(".order-btn").addEventListener("click", () => addToCart(product));
    ...
    function updateCartUI() {
        ...
        document.querySelectorAll(".qty-increase").forEach(btn => {
            btn.addEventListener("click", () => {
                ...
                const item = cart.find(p => p.productName === name);
                if (item) {
                    cart.push(item);
                    localStorage.setItem("cart", JSON.stringify(cart));
                    updateCartUI();
